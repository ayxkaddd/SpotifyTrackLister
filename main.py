...
oauth = spotipy.Spotify(auth_manager=SpotifyOAuth(client_id=client_id, client_secret=client_secret, scope=scope, redirect_uri=redirect_uri))
...
def get_tracks(offset):
    ...
    tracks_file = open("tracks.txt", "a", encoding="utf-8")
    for item in items:
        begin += 1
        ...
        print(f"[{begin}] - {artist_name} - {track_name} to tracks.txt")
        end = begin
